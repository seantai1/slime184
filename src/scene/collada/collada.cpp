    ...
    if (tech_CGL) {
      ...
      while (e_bsdf) {
        ...
        if (type == "emission") {
          ...
          XMLElement *e_phase_g = e_bsdf->FirstChildElement("phase_g");
          if (e_sigma_a && e_sigma_s) {
            Vector3D sigma_a = spectrum_from_string(string(e_sigma_a->GetText()));
            Vector3D sigma_s = spectrum_from_string(string(e_sigma_s->GetText()));
            double g = e_phase_g ? atof(e_phase_g->GetText()) : 0.0;
            bsdf->medium_inside = new HomogeneousMedium(sigma_a, sigma_s, g);
            stat("  |- SSS medium: sigma_a=" << sigma_a
                 << " sigma_s=" << sigma_s << " g=" << g);
          }
          material.bsdf = bsdf;
        }
