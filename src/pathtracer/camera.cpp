...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double sensorX = -tan(hFovRad / 2.0) + x * (tan(hFovRad / 2.0) - (-tan(hFovRad / 2.0)));
  double sensorY = -tan(vFovRad / 2.0) + y * (tan(vFovRad / 2.0) - (-tan(vFovRad / 2.0)));

  Vector3D dirCamera(sensorX, sensorY, -1.0);
  dirCamera.normalize();
  // rotation matrix
